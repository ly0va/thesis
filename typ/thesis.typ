...
+ *Нульове розголошення* (Zero knowledge): Верифікатор не дiзнається жодної iнформацiї, окрім факту істинності твердження.
...
+ *Підготовка*: Перед початком протоколу сторона, яка доводить знання ключа (прувер), та сторона, якій доводиться (верифікатор), домовляються про певне велике просте число $p$ і примітивний корінь $g$ по модулю $p$. Верифікатор знає $y = g^x mod p$. Прувер хоче довести, що знає $x$, залишивши його в секреті.
...
жодної додаткової інформації про цей секрет.

Цей протокол використовує криптографічне припущення про нерозв'язність DLP.

\
=== Обґрунтування протоколу Шнорра
\

Нехай існує алгоритм, що може переконати Верифікатора, що він знає $x$ не маючи його.
Тоді нехай цей алгоритм успішно пройшов два виклики $c_1, c_2$ Верифікатора для фіксованого $x$ --
тобто відправив $s_1, s_2$. Тоді
$
  s_1 - s_2 mod p-1 = c_1 x - c_2 x mod p-1 \
  => (s_1 - s_2)/(c_1 - c_2) mod p-1 = x mod p-1
$
Отже, цей алгоритм еквівалентний добуванню дискретного логарифму з
$y = g^x = g^((s_1 - s_2)/(c_1 - c_2))$, що і треба було довести.
...
+ *Кільцеві Підписи* (_Ring Signatures_): використовуються для забезпечення анонімності відправника. Вони дозволяють відправнику підписати транзакцію з групою можливих підписувачів, тим самим забезпечуючи анонімність, оскільки неможливо визначити, хто з групи дійсно є відправником.
...
криптографічні примітиви, що використовуються в MimbleWimble.
...
- `Tornado`, що буде мати інтерфейс для депозитів та виводів, а також буде підтримувати дерево Меркла (описано в @merkle-tree[розділі]) з комітментів користувачів.
...
#tab Варто зазначити, що контракт зберігає декілька попередніх кореневих хешів на випадок, якщо
кілька користувачів одночасно хочуть зробити депозит та вивести кошти. Без цього, депозит
змінив би хеш, транзакція виводу провалилась би, користувач втратив би гроші, витрачені на комісію
та був би вимушений генерувати новий доказ.

У контракті `Hasher` xеш MiMC @mimc використовується через його низьку мультиплікативну
...
транзакції можна буде відслідкувати за їх метаданими, що отримують вузли Ethereum чи інтернет-провайдери
...
Як і всі інші DeFi протоколи, включаючи той, що описаний в даній роботі,
Aave має кілька потенційних ризиків, які користувачам варто враховувати. Ось декілька з них:
...
Весь вихідний код протоколу, тести та скрипти, використані для симуляцій
опубліковані на GitHub @this під ліцензією MIT.
...
У даній роботі було детально розглянуто властивості різних
протоколів доказів з нульовим розголошенням, наведені приклади сучасних
протоколів та складено їх порівняльну характеристику. Також, було
проведено огляд різних протоколів для анонімних переказів та описано
технології, що вони використовують, їх переваги та недоліки.

Основну мету -- покращення реалізації існуючого протоколу для анонімних
переказів на основі доказів з нульовим розголошенням -- виконано.
Протокол реалізовано на сучасній маловідомій мові програмування
та з використанням сучасної пруф-системи, що збільшило їх адопцію
та буде прикладом їх використання для майбутніх розробників.
Реалізовано покращення, що включають інтеграцію
протоколу Aave та функцію миттєвих позик. Оцінено їхню ефективність --
розраховані потенційні прибутки для протоколу після впровадження
даних вдосконалень.

Весь вихідний код протоколу, тести та скрипти, використані для симуляцій
опубліковані на GitHub під ліцензією MIT. Протокол впроваджено на
тестову мережу Goerli для публічного тестування.

Дослідження, проведені у роботі, можуть бути використані іншими розробниками
протоколів, фін-тех компаніями та фінансовими установами. Сам протокол
може використовуватись користувачами, що бажають зберігти ефективність
капіталу під час анонімних переказів.

Подальші дослідження за цією темою можуть розглядати інтеграцію
інших DeFi інструментів у протоколи анонімних переказів, методи зменшення ризиків
та оптимізацію обчислень, виконуваних на смарт-контрактах.

#pagebreak()

= Перелік джерел посилання

#show "Online": "Електронний ресурс"
#show "Available": "Режим доступу до ресурсу"
#show regex("\[\d+\]"): it => it.text.trim("[").replace("]", ".")
